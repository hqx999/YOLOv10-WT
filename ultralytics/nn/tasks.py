...
try:
    import sys, os
    custom_path = os.path.join(os.path.dirname(__file__), '..', '..', 'custom_lib', 'modules')
    sys.path.insert(0, os.path.abspath(custom_path))

    def import_custom_modules():
        from custom_lib.modules.custom_modules_ds import (
            PConv, WTConv, ECA, GatedSpatialConv,
            C2f_WT_Light, SPPF, v10Detect
        )
        return PConv, WTConv, ECA, GatedSpatialConv, C2f_WT_Light, SPPF, v10Detect

    PConv, WTConv, ECA, GatedSpatialConv, C2f_WT_Light, SPPF, v10Detect = import_custom_modules()
    print("✓ Custom modules imported successfully in tasks.py")
except ImportError as e:
    print(f"✗ Failed to import custom modules in tasks.py: {e}")
    PConv = WTConv = ECA = GatedSpatialConv = C2f_WT_Light = SPPF = v10Detect = None
# === 结束自定义模块导入 ===
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        if isinstance(m, (v10Detect,)):
            ...
            with torch.no_grad():
                ...
                # 只做一次 predict，不要再调用 self.forward() 以免递归
                pred = self._predict_once(dummy)

                # 1) 如果返回 dict，先从里面取第一个 Tensor
                if isinstance(pred, dict):
                    tensor_list = [v for v in pred.values() if isinstance(v, torch.Tensor)]
                    if len(tensor_list) == 0:
                        # 兜底：默认步长
                        m.stride = torch.tensor([8.0, 16.0, 32.0])
                        LOGGER.warning(f"v10Detect returned dict without tensor, using default stride: {m.stride}")
                    else:
                        pred = tensor_list  # 作为列表继续往下走

                # 2) 如果返回 list/tuple（典型的 [P3,P4,P5]）
                if isinstance(pred, (list, tuple)):
                    tensor_list = [p for p in pred if isinstance(p, torch.Tensor)]
                    if len(tensor_list) >= 1 and tensor_list[0].dim() == 4:
                        # 按每个特征图的空间尺寸分别计算步长
                        m.stride = torch.tensor([s / p.shape[-2] for p in tensor_list], dtype=torch.float)
                    else:
                        m.stride = torch.tensor([8.0, 16.0, 32.0])
                        LOGGER.warning(f"Unexpected list prediction format, using default stride: {m.stride}")

                # 3) 如果返回单个 Tensor（B,C,H,W）
                elif isinstance(pred, torch.Tensor) and pred.dim() == 4:
                    m.stride = torch.tensor([s / pred.shape[-2]], dtype=torch.float)
...
def parse_model(d, ch, verbose=True):
    ...
    # === 调试打印 ===
    print("\n" + "=" * 60)
    print("=== parse_model 开始解析 ===")
    print(f"输入通道数 ch: {ch}")
    print(
        f"模型配置 nc: {d.get('nc')}, depth_multiple: {d.get('depth_multiple')}, "
        f"width_multiple: {d.get('width_multiple')}"
    )
    print(f"Backbone 层数: {len(d['backbone'])}, Head 层数: {len(d['head'])}")

    for name in ['PConv', 'WTConv', 'ECA', 'GatedSpatialConv', 'C2f_WT_Light', 'SPPF', 'v10Detect']:
        ok = (name in globals()) and (globals()[name] is not None)
        print(f"{'✓' if ok else '✗'} {name} 在全局命名空间中" if ok else f"✗ {name} 不在全局命名空间中或为 None")
    print("=" * 60 + "\n")
    ...
    base_modules = frozenset({
        ...
        SCDown, C2fCIB, A2C2f,
        # 你的自定义：和卷积/块族同样处理（会缩放 c2）
        PConv,
        C2f_WT_Light,  # 作为 C2f 家族处理（会注入 repeat n）
    })
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):
        ...
        if m in base_modules:
            # 统一基础模块：替换 c1/c2，其它参数原样保留
            c1, raw_c2 = ch[f], args[0]
            c2 = make_divisible(min(raw_c2, max_channels) * width, 8) if raw_c2 != nc else raw_c2
            ...
            # === GhostConv 参数清洗（避免把奇怪的“24”当作 k / groups）===
            if m is GhostConv:
                # 如果第三位不是常见的 k 值，删除它（把它当作 YAML 里误写的占位/比率）
                if len(args) >= 4 and isinstance(args[2], int) and args[2] not in (1, 3, 5, 7):
                ...
                if len(args) == 3:
                    args += [1]
                # groups 合法性兜底
                if len(args) >= 5 and isinstance(args[4], int) and args[0] % args[4] != 0:
                    args[4] = 1

            # === C2f_WT_Light 参数清洗（去掉重复 c2 与把 n 错写进 args 的情况）===
            if m is C2f_WT_Light:
                # 当前 args 形如 [c1, c2, *rest]
                rest = args[2:]
                # 1) 有些 YAML 写成 [c2, c2, n]，会导致 rest[0] == c2
                if len(rest) >= 1 and isinstance(rest[0], int) and rest[0] == c2:
                    rest = rest[1:]
                # 2) 有些把 n 也写进了 args（例如 [c2, n, False]），
                #    由于 parse_model 会注入 n，这里把“领先的整型 n”剔掉，保留后面的 shortcut 等布尔量
                if len(rest) >= 1 and isinstance(rest[0], int):
                    rest = rest[1:]
                args = [args[0], args[1], *rest]
            ...
            print(f"  基础模块处理: c1={c1}, c2={c2}, 最终参数: {args}")
            ...
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB}:
                m.legacy = legacy
            c2 = ch[f if isinstance(f, int) else f[-1]]

        elif m is RTDETRDecoder:
            args.insert(1, [ch[x] for x in f])

        elif m is CBLinear:
            c2 = args[0]; c1 = ch[f]; args = [c1, c2, *args[1:]]

        elif m is CBFuse:
            c2 = ch[f[-1]]

        elif m in frozenset({TorchVision, Index}):
            c2 = args[0]; c1 = ch[f]; args = [*args[1:]]
